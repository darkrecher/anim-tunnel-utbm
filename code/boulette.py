...
class Boulette(AnimationObject):
    ...
    def __init__(self):
        ...
        for img_index in range(Boulette.NB_IMG_BOULETTES):
            img_boulette, boul_rect = load_image("boulette_0" + str(img_index+1) + ".png", use_alpha=True)
    ...
    def _change_polar_coords(self):
        ...
        if self.delta_angle < -Boulette.DELTA_ANGLE_MAX and accel_angle < 0:
        ...
        # Mais osef.
        if self.delta_dist < -Boulette.DELTA_DIST_MAX and accel_dist < 0:
            self.delta_dist -= accel_dist * Boulette.ACCEL_DIST_LIMITING_FACTOR
        elif self.dist < Boulette.DIST_MIN and accel_dist < 0:
            self.delta_dist -= accel_dist * Boulette.ACCEL_DIST_LIMITING_FACTOR
        if self.delta_dist > Boulette.DELTA_DIST_MAX and accel_dist > 0:
            self.delta_dist -= accel_dist * Boulette.ACCEL_DIST_LIMITING_FACTOR
        elif self.dist > Boulette.DIST_MAX and accel_dist > 0:
            self.delta_dist -= accel_dist * Boulette.ACCEL_DIST_LIMITING_FACTOR
    ...
    def _get_img_and_coords(self):
        ...
        index_boulette_rotation = int(( (360 -(self.angle -Boulette.IMG_ROTATION_PRECISION/2 -90)) / 360) * self.len_boulette_imgs_rot)
